...
// Updated to preserve color by keeping original UV planes
//
// Build:
...
static gpointer worker_thread_fn(gpointer user_data) {
    ...
    while (!d->stop.load(std::memory_order_acquire)) {
        ...
        try {
            ...
            // Histogram Equalization on Y channel only
            ...
            // Map output buffer and reconstruct NV12 with original UV planes
            GstMapInfo out_map_info;
            if (gst_buffer_map(outbuf, &out_map_info, GST_MAP_WRITE)) {
                // Copy processed Y plane (luminance)
                memcpy(out_map_info.data, y_plane_out.data, y_size);
                
                // KEEP COLOR: Copy original UV planes (chrominance) from input
                // This preserves the original color information while only enhancing brightness/contrast
                memcpy(out_map_info.data + y_size, map_info.data + y_size, uv_size);
...
int main(int argc, char *argv[]) {
    ...
    g_print("Encoder: %s, target-bitrate: %d kbps, workers: %d, %dx%d@%dfps (COLOR OUTPUT)\n",
    ...
    g_print("OpenCV histogram equalization pipeline started with COLOR output. Press Ctrl+C to exit.\n");
