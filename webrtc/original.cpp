...
static void on_ws_message(SoupWebsocketConnection*,
  ...
  if (g_strcmp0(type_str, "connected") == 0) {
    ...
    // if (app->ws_ready && !app->offer_sent) create_and_send_offer(app);
      // A viewer just joined; (re)send an offer so they actually receive it.
  app->offer_sent = false;
  if (app->ws_ready) create_and_send_offer(app);
...
//   ss
//     << "v4l2src device=" << app->device
//     << " ! video/x-raw,format=NV12,width=" << app->width << ",height=" << app->height
//     << ",framerate=" << app->fps << "/1 ";

//   if (app->codec == Codec::H264) {
//     // Use the proven OMX settings from your working pipeline
//     ss << " ! omxh264enc num-slices=8 periodicity-idr=240 cpb-size=500 "
//        << "gdr-mode=horizontal initial-delay=250 control-rate=low-latency "
//        << "prefetch-buffer=true target-bitrate=" << app->bitrate_kbps
//        << " gop-mode=low-delay-p "
//        << " ! video/x-h264,alignment=nal "
//        << " ! rtph264pay config-interval=1 pt=96 "
//        << " ! application/x-rtp,media=video,encoding-name=H264,payload=96 ";
//   } else { // H265
//     ss << " ! omxh265enc num-slices=8 periodicity-idr=240 cpb-size=500 "
//        << "gdr-mode=horizontal initial-delay=250 control-rate=low-latency "
//        << "prefetch-buffer=true target-bitrate=" << app->bitrate_kbps
//        << " gop-mode=low-delay-p "
...
//   ss << " ! webrtcbin name=sendrecv bundle-policy=max-bundle";
//   return ss.str();
// }
...
//     // control-rate=1 is variable bitrate, which is good for WebRTC.
//     ss << "! omxh264enc target-bitrate=" << (app->bitrate_kbps * 1000) // Bitrate in bps
...
//        << "! application/x-rtp,media=video,encoding-name=H264,payload=96 ";
//   } else { // H265
//     // H.265 can work but is not universally supported in browsers via WebRTC.
//     ss << "! omxh265enc num-slices=8 periodicity-idr=240 cpb-size=500 gdr-mode=horizontal initial-delay=250"
//        << " control-rate=low-latency prefetch-buffer=true target-bitrate=" << (app->bitrate_kbps * 1000)
//        << " gop-mode=low-delay-p "
//        << " ! video/x-h265,alignment=nal "
//        << " ! rtph265pay config-interval=1 pt=96 "
//        << "! application/x-rtp,media=video,encoding-name=H265,payload=96 ";
...
static std::string build_pipeline_str(App* app) {
  ...
  if (app->codec == Codec::H264) {
    // Use proven OMX settings with explicit packetization mode
    ss << " ! omxh264enc target-bitrate=" << app->bitrate_kbps * 1000 
       << " control-rate=low-latency gop-mode=low-delay-p periodicity-idr=240 "
    ...
    ss << " ! omxh265enc target-bitrate=" << app->bitrate_kbps * 1000 
       << " control-rate=low-latency gop-mode=low-delay-p periodicity-idr=240 "
       << " ! video/x-h265,stream-format=byte-stream,profile=main "
